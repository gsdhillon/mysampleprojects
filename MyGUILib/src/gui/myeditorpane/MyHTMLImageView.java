...
/**
 * MyHTMLImageView.java
 */
public class MyHTMLImageView extends View  implements Serializable{
     ...
     * @param elem
     */
    public MyHTMLImageView(Element elem) {
        super(elem);
    ...
    private void loadImage() throws Exception{
        ...
        if (src != null) {
            ...
            String src1 = (String) getElement().getAttributes().getAttribute(HTML.Attribute.SRC);
            if (src1 != null) {
                src1 = src1.substring(src1.indexOf(",") + 1);
                    //byte[] bytebuff = (new BASE64Decoder()).decodeBuffer(src1);
                //changed on 19-02-2012 by gurmeet
                byte[] bytebuff = Base64Tool.base64Decode(src1);
                newImage = Toolkit.getDefaultToolkit().createImage(bytebuff);
                if (newImage != null && getLoadsSynchronously()) {
                    // Force the image to be loaded by using an ImageIcon.
                    ImageIcon ii = new ImageIcon();
                    ii.setImage(newImage);
    ...
    private class ImageHandler implements ImageObserver {
        ...
        public boolean imageUpdate(Image img, int flags, int x, int y,
            ...
            if ((flags & (ABORT | ERROR)) != 0) {
                repaint(0);
                synchronized (MyHTMLImageView.this) {
                    if (image == img) {
