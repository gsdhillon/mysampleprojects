 ...
 * SignedForms.java
 */
public class SignedForms extends MyApplet implements
    ...
    public void init() {
        try {
            ...
            MyUtils.showException("SignedForms", e);
        }
    }
    ...
    private void viewSelectedDoc() {
        ...
        if (docType.equals(VisitorForm.shortName)) {
            viewPanel = new ViewVisitorForm(formID, "AUTH", this);
        }
        //TODO NEW_APP
        if (viewPanel != null) {
            contentPane.removeAll();
    ...
    private class DocumentsTableModel extends MyTableModel {
        ...
        public DocumentsTableModel() throws Exception {
            ...
            //Column 2
            addColumn(new MyTableColumn("SignDate", 100, MyTableColumn.TYPE_DATE_SHORT) {
        ...
        public void getData() {
            try {
                rowCount = 0;
                String docType = ((String) docTypeCombo.getSelectedItem()).trim();
                MyHTTP myHTTP = MyUtils.createServletConnection("DigSignFormListServlet");

                myHTTP.openOS();
                myHTTP.println("getsigneddoclist");
