 ...
 */
public class EarlyLateGUI extends MyPanel implements ActionListener {
    ...
    public JCheckBox chkTodays;
    private MyPanel panFrmDt;
    private MyPanel panToDt;
    ...
    public EarlyLateGUI(MyApplet applet, String ReportName, String empCode) {
        this.applet = applet;
    ...
    private void addMusterPanel() {
        try {
            ...
            panMainNorth1 = new MyPanel(new GridLayout(1, 5), "Select Date");
            MyLabel lblfill1 = new MyLabel();
            panMainNorth1.add(lblfill1);

            panFrmDt = new MyPanel(new GridLayout(1, 1), "From");
            ...
            MyPanel panToday = new MyPanel(new FlowLayout(FlowLayout.CENTER));
            chkTodays = new JCheckBox("Today's");
            chkTodays.addActionListener(this);
            chkTodays.setActionCommand("ToaysReport");
            panToday.add(chkTodays);
            panMainNorth1.add(panToday);

            panToDt = new MyPanel(new GridLayout(1, 1), "To");
            ...
            MyUtils.showException("EarlyLate", ex);
        }
    }

    private boolean FormFilled() {
        if (chkTodays.isSelected() == false) {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand().equals("ToaysReport")) {
            if (chkTodays.isSelected() == true) {
                panFrmDt.setVisible(false);
                panToDt.setVisible(false);
                Todays = true;
            } else {
                panFrmDt.setVisible(true);
                panToDt.setVisible(true);
                Todays = false;
            }
        }
    ...
    private void View() {
        if (FormFilled()) {
            try {
                ...
                if (chkTodays.isSelected() == true) {
                    Todays = true;
                } else if (chkTodays.isSelected() == false) {
                    Todays = false;
                }
                String ReportType = "";
                CreatePacketEarlyLate packet;
                ...
                if (rbSel.isSelected() == true) {
                    ...
                    packet = new CreatePacketEarlyLate(this, cmbdiv1, cmbSec1, cmbWL1, Todays);
                    ...
                    CreatePacketEarlyLate divPacket;
                    if (!"DivHead".equals(strHead) && !"Both".equals(strHead)) {
                        DivName = new String[cmbDiv.getItemCount()];
                        divPacket = new CreatePacketEarlyLate(getComboItems(cmbDiv, cmbDiv.getItemCount()));
                        packet = new CreatePacketEarlyLate(this, strHead, Todays);//sends section wise Division
                    } else {
                        DivName = new String[cmbDiv.getItemCount()];
                        divPacket = new CreatePacketEarlyLate(getComboItems(cmbDiv, cmbDiv.getItemCount()));
                        packet = new CreatePacketEarlyLate(this, strHead, Todays);//sends Division if emp is divhead
                    }
    ...
    private void callServlet(String Packet, String divPacket, String reqType, String ReportType, String rptWise) {
        ...
        if (!Packet1.equals("PacketFail")) {
            try {
                MyHTTP myHTTP = MyUtils.createServletConnection("EarlyLateFormSevlet");
                myHTTP.openOS();
                ...
                if (result.startsWith("ERROR")) {
                    ...
                    if ("EarlyReport".equals(reqType)) {
                        ((EarlyApplet) applet).showReportPanel(Packet, result, divPacket, rptWise);
                    } else if ("LateReport".equals(reqType)) {
                        ((LateApplet) applet).showReportPanel(Packet, result, divPacket, rptWise);
                    }
                }
            } catch (Exception ex) {
