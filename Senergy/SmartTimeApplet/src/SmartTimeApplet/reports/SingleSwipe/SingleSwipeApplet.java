 ...
 * @author pradnya
 */
public class SingleSwipeApplet extends MyApplet {
    ...
    public void init() {
        try {
            ...
            SingleSwipeGUI RG = new SingleSwipeGUI(this, "SingleSwipe", empCode);
//            if (!RG.checkHead(empCode, "Section") && (!RG.checkHead(empCode, "Division"))) {
//                showSelfReport();
//            } else {
                LoadPanel(RG);
//            }
        } catch (Exception e) {
            MyUtils.showException("SingleSwipe", e);
        }
    }
    ...
    public void showReportPanel(String Packet, String result, String divNamePacket, String rptType, String reqType) {
        try {

            Depacketizer d = new Depacketizer(result);
            int rowCount = d.getInt();

            ReportTableView rtv = new ReportTableView(this, Packet, divNamePacket, reqType, rowCount, rptType);
            ...
            ReportClass[] SingleSW = new ReportClass[rowCount];
            //get all doc
            for (int i = 0; i < rowCount; i++) {
                SingleSW[i] = new ReportClass();
                for (int j = 0; j < col; j++) {
                    SingleSW[i].returnVals[j] = d.getString();
                }
            }
            tablemodel.setData(SingleSW);
    ...
    private void showSelfReport() {
        try {
            ...
            contentPane.add(new SelfReport(this, "SingleSwipe"), BorderLayout.CENTER);
    ...
    public void showMonthlyMuster(int index) {
        ...
        contentPane.add(new MonthlyMuster("SingleSwipe", index), BorderLayout.CENTER);
        contentPane.validate();
        contentPane.repaint();
    }
}
