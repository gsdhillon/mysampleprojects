...
public class PKITasks {
     ...
     * @return - response String
     */
    public static String backupCertificate(String empno) {
        try {
            MSC msc = new MSC("PKIServlet");
            PrintStream ps = msc.getPS();
            ps.println("backup_certificate");
            ps.println(empno);
            ps.close();
            BufferedReader br = msc.getBR();
            String response = br.readLine();
            ...
            MyUtils.showException("while backup certificate of empno", ex);
            return "ERROR:"+ex.getMessage();
        }
    }
    /**
